...
function NavItem({ label, section, current, onClick }: { label: string; section: Section; current: Section; onClick: () => void }) {
...
export default function Cuenta() {
  ...
  if (!isAuthenticated) return <Navigate to="/login" state={{ from: '/cuenta' }} replace />
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="lg:w-56 shrink-0" aria-label="Navegación de cuenta">
          ...
          <div className="text-center mb-6 lg:block hidden">
            ...
            <p className="font-bold text-main-dark text-sm">{user?.firstName} {user?.lastName}</p>
        ...
        <main className="flex-1 min-w-0">
          ...
          {section === 'overview' && (
            <section aria-labelledby="overview-title">
              <h1 id="overview-title" className="text-2xl font-black text-main-dark mb-2">Resumen de cuenta</h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                {[
                  ...
                  { label: 'Pedidos activos', value: MOCK_ORDERS.filter(o => o.status === 'En camino').length, icon: '🚴' },
                  { label: 'Gastado total', value: formatPrice(MOCK_ORDERS.reduce((s, o) => s + o.total, 0), currency), icon: '💳' },
                ].map((stat) => (
                  <div key={stat.label} className="bg-white rounded-2xl p-5 shadow-sm border border-secondary-gray/15">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-secondary-gray/15 overflow-hidden">
                <div className="flex items-center justify-between p-5 border-b border-secondary-gray/15">
                  ...
                  <button onClick={() => setSection('orders')} className="text-sm text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded">
                    Ver todos
                  </button>
                </div>
                      ...
                      {MOCK_ORDERS.slice(0, 3).map((order) => (
                        <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-5 py-3.5">
                            <span className={`inline-block text-xs font-semibold px-2.5 py-0.5 rounded-full ${STATUS_COLORS[order.status] || 'bg-slate-100 text-slate-600'}`}>
                              {order.status}
                            </span>
                          </td>
                          <td className="px-5 py-3.5 text-right font-bold text-main-dark">{formatPrice(order.total, currency)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {section === 'orders' && (
            <section aria-labelledby="orders-title">
              <h1 id="orders-title" className="text-2xl font-black text-main-dark mb-6">Mis pedidos</h1>
          ...
          {section === 'addresses' && (
            <section aria-labelledby="addresses-title">
              <h1 id="addresses-title" className="text-2xl font-black text-main-dark mb-6">Mis direcciones</h1>
              <div className="text-center py-16 bg-white rounded-2xl border border-secondary-gray/15">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-secondary-gray/30 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
          ...
          {section === 'profile' && (
            <section aria-labelledby="profile-title">
              <h1 id="profile-title" className="text-2xl font-black text-main-dark mb-6">Mis datos</h1>
              <div className="bg-white rounded-2xl shadow-sm border border-secondary-gray/15 p-6">
                <form onSubmit={handleSubmit(onSave)} noValidate className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="p-firstName" className="block text-sm font-semibold text-main-dark mb-1.5">Nombre</label>
                      <input id="p-firstName" type="text" className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary ${errors.firstName ? 'border-red-400' : 'border-secondary-gray/30'}`} {...register('firstName')} />
                      {errors.firstName && <p role="alert" className="mt-1 text-xs text-red-500">{errors.firstName.message}</p>}
                    </div>
                    <div>
                      <label htmlFor="p-lastName" className="block text-sm font-semibold text-main-dark mb-1.5">Apellido</label>
                      <input id="p-lastName" type="text" className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary ${errors.lastName ? 'border-red-400' : 'border-secondary-gray/30'}`} {...register('lastName')} />
                      {errors.lastName && <p role="alert" className="mt-1 text-xs text-red-500">{errors.lastName.message}</p>}
                    </div>
                  </div>
                  <div>
                    <label htmlFor="p-email" className="block text-sm font-semibold text-main-dark mb-1.5">Email</label>
                    <input id="p-email" type="email" className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary ${errors.email ? 'border-red-400' : 'border-secondary-gray/30'}`} {...register('email')} />
                  ...
                  <div>
                    <label htmlFor="p-phone" className="block text-sm font-semibold text-main-dark mb-1.5">
                      Teléfono <span className="text-secondary-gray font-normal">(opcional)</span>
                    </label>
                    <input id="p-phone" type="tel" placeholder="+54 11 ..." className="w-full px-4 py-3 border border-secondary-gray/30 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary" {...register('phone')} />
                  </div>

  ...
  return (
    <div className="flex-1 flex items-center justify-center py-20">
      <div className="text-center max-w-sm">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-20 w-20 text-secondary-gray/20 mx-auto mb-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
        </svg>
        <h2 className="text-xl font-bold text-main-dark mb-2">Tu carrito está vacío</h2>
        <p className="text-secondary-gray text-sm mb-6">Agregá productos desde nuestro catálogo.</p>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <section className="flex-1" aria-label="Productos en el carrito">
          <ul className="space-y-4" role="list">
            {items.map((item) => (
              <li key={item.id} className="bg-white rounded-2xl p-5 flex items-center gap-5 shadow-sm">
                <div className="w-20 h-20 bg-slate-100 rounded-xl overflow-hidden shrink-0">
                  <img src={item.image} alt={item.name} className="w-full h-full object-cover" loading="lazy" />
                </div>

                <div className="flex-1 min-w-0">
                  <Link to={`/producto/${item.slug}`} className="font-semibold text-main-dark text-sm hover:text-primary transition-colors line-clamp-2 focus-visible:outline-none focus-visible:underline">
                    {item.name}
                  </Link>
                  <p className="text-primary font-black mt-1">{formatPrice(item.price, currency)}</p>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <div className="flex items-center border border-secondary-gray/30 rounded-xl overflow-hidden text-sm">
                    ...
                    >–</button>
                    <span className="px-3 font-semibold min-w-[2rem] text-center" aria-live="polite">{item.quantity}</span>
                    <button
                      onClick={() => updateQuantity(item.id, item.quantity + 1)}
                      className="px-3 py-1.5 hover:bg-slate-100 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          ...
          <Link to="/catalogo" className="inline-flex items-center gap-2 text-sm text-secondary-gray hover:text-primary transition-colors mt-6 focus-visible:outline-none focus-visible:underline">
        ...
        <aside className="lg:w-80 shrink-0" aria-label="Resumen del pedido">
          <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
            <h2 className="font-bold text-main-dark text-lg mb-5">Resumen del pedido</h2>
            ...
            <dl className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className={shipping === 0 ? 'text-primary font-semibold' : 'font-semibold'}>
                  {shipping === 0 ? 'Gratis' : formatPrice(shipping, currency)}
                </dd>
              </div>
              {shipping > 0 && (
                <p className="text-xs text-secondary-gray bg-slate-50 rounded-lg p-2">
                  Agregá {formatPrice(SHIPPING_THRESHOLD - subtotal, currency)} más para envío gratis.
                </p>
              )}
            ...
            <div className="flex items-center justify-center gap-4 mt-5 pt-5 border-t border-secondary-gray/15">
              <div className="flex items-center gap-1 text-xs text-secondary-gray">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
